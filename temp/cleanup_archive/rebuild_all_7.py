...
# Read current file to preserve preamble and other parts
with open('FORMLÆRE.tex', 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Update Preamble - Fancyhdr and Footnote rule
# We want header rule and footer rule to match. Footnote rule invisible.
...
# 2. Reconstruct Chapters 1-7 from provided text
new_main_matter = r"""\mainmatter
...
\prop{3.1212}{t}{Eit landskap with éin global optimal er eit spesialtilfelle, ikkje det generiske tilfellet. Funksjonalismens implisitte føresetnad om éin optimal løysing er empirisk feil for dei fleste funksjonelle klasser.}
...
\prop{5.2332}{t}{Sidan lyskjegla avgjer kva innleiringar agenten kan oppdage, og sidan innleiringa avgjer kva reglar som kan fyre, er den generative kapasiteten avgrensa av lyskjegla. Two agentar med same grammatikk men ulike lyskjeglar produserer ulike former.}
...
if mainmatter_start != -1 and appendix_start != -1:
    content = content[:mainmatter_start] + new_main_matter + "\n\n" + content[appendix_start:]

# Fix specific typos from the prompt text
...
content = content.replace("in ein etikett", "i ein etikett")
content = content.replace("with éin global", "med éin global")

# Remove "C-K-teorien" mentions as instructed
content = content.replace("C-K-teorien", "Intensjonal navigasjon")
