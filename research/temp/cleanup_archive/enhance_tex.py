...
replacements_substr = {
    ...
    r"formrommet (morphospace) til ein klasse er mengda av alle moglege konfigurasjonar for objekt i denne klassen.}": r"formrommet (morphospace) til ein klasse er mengda av alle moglege konfigurasjonar for objekt i denne klassen.\footnote{Raup (1966); Mitteroecker \& Huttegger (2009)}}",
    r"Inga endeleg mengd parametrar captures den latente kompleksiteten fullt ut.}": r"Inga endeleg mengd parametrar fangar den latente kompleksiteten fullt ut.\footnote{Thompson (1917)}}", # Also fix 'captures' -> 'fangar'
...
# Minor language fixes to adhere strictly to prosa.md
content = content.replace("captures", "fangar")
content = content.replace("zoom", "rørsle") # just in case

with open('FORMLÆRE.tex', 'w', encoding='utf-8') as f:
    f.write(content)

print("Enhancement complete. File written to FORMLÆRE.tex")