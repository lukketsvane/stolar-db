...
# Read current file to preserve preamble
with open('FORMLÆRE.tex', 'r', encoding='utf-8') as f:
    content = f.read()

# 1. PREAMBLE AND STYLING FIXES (Consolidated)
# Ensure header rule is correct and footer line is removed.
preamble_updates = r"""\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhf{}
...
# Replace the fancyhdr/footnote block in preamble
fancy_pattern = re.compile(r"\\usepackage\{fancyhdr\}.*?\\renewcommand\{\\footnoterule\}\{\}", re.DOTALL)
if fancy_pattern.search(content):
    content = fancy_pattern.sub(preamble_updates, content)
else:
    # Fallback if the pattern differs slightly
...
# 2. RECONSTRUCT MAIN CONTENT WITHOUT FORCED PAGE BREAKS
...
# Helper to add a transition that doesn't force a new page unless necessary
def chap_header(num, title):
    mark = title.lower()
    return rf"""\par\addvspace{24pt}
\addcontentsline{{toc}}{{chapter}}{{{num} {title}}}
\markboth{{{mark}}}{{}}
\prop{{{num}}}{{{title}}}"""

# Note: The provided text used "konfigurasjon" but the user said they don't like it.
# I will replace "konfigurasjon" with "posisjon" throughout.
...
\prop{1.1}{o}{Formverda er totaliteten av realiserte \textbf{posisjonar}, ikkje av ting.}

\prop{1.11}{t}{Alt som har form, har nettopp denne forma og ikkje ei anna.}

\prop{1.111}{t}{At ein \textbf{posisjon} tek éin eksakt form, når uendeleg mange andre var logisk moglege, krev ei årsaksforklaring utover \textbf{posisjonen} sjølv.}

\prop{1.1111}{t}{Forklaringa ligg i relasjonen mellom den manifesterte \textbf{posisjonen} og totaliteten av dei formene som var moglege men ikkje vart realiserte. Det er ein relasjonell, ikkje ein absolutt, eigenskap.}

\prop{1.112}{d}{Eit \textbf{objekt} er den enklaste bestanddelen i ein form. Det er udeleleg og uforanderleg.}

\prop{1.1121}{t}{Sidan objektas natur inneber moglegheita for å inngå i \textbf{posisjonar}, er alle moglege former allereie gjeve i og med objekta.}

\prop{1.1122}{o}{Moglegheita er logisk tilgjengeleg fordi objekta er der, ikkje fordi nokon tenkjer ho.}

\prop{1.12}{d}{Ein \textbf{posisjon} er ein bestemt samanheng av objekt. Forma er \textbf{posisjonens} struktur.}

\prop{1.2}{d}{\textbf{Formrommet} (\textit{morphospace}) til ein klasse er mengda av alle moglege \textbf{posisjonar} for objekt i denne klassen.\footnote{Raup (1966); Mitteroecker \& Huttegger (2009)}}
...
\prop{2.1411}{i}{Stolen og sofaen har ulik funksjon og utgjer ulike klasser. To stolar med ulik sitte-stil har same funksjon. All skilnaden mellom dei er ikkje-funksjonell.}
...
\prop{2.221}{o}{Stilperiode er ein slik samlevariabel. Han er ikkje ein forklaring; han er ein proxy som absorberer alle samtidige trykk i éin etikett.}
...
\prop{3.111}{o}{Vektene are ikkje frie parametrar; dei are empirisk tilgjengelege som samvariasjonen mellom kvart trykk og den observerte fordelinga, betinga på alle andre trykk.}
...
\prop{3.1113}{o}{Prediksjonshorisonten er ein funksjon av landskapets endringsrate. I periodar med stase er prediksjonen lang; i bifurkasjonar kort.}
...
\prop{3.21}{t}{Nokre seleksjonstrykk er sterkare enn andre. Dei sterkaste kanaliserer formrommet i få retningar.}
...
\prop{5.1121}{o}{Definisjonen er ikkje vakuøs; ho er stratifisert. Differansen mellom ein termostat og ein meisterhåndverkar ligg ikkje i substrat, men i kompleksiteten i navigasjonskapasiteten.}
...
\prop{5.1211}{t}{To agentar produserer same resultat om dei registrerer same lokale gradient, uavhengig av om dei har nokon representasjon av det globale landskapet. Kompetansen er lokal; effekten er global.}
...
\prop{5.211}{o}{Lyskjegla avgrensar oppløysinga: kva delformer ein agent kan identifiserere, avgjer kva transformasjonar som er tilgjengelege.}
...
\prop{5.22}{d}{Fem operasjonar modifiserer lyskjegla: utviding, oppløysingsauke, rørsle, kollaps, skjerping.}
...
\prop{5.25}{t}{Navigasjonskompetanse is akkumulert. Kvar realisert form utvidar agentens lyskjegle. Læring er den temporale utvidinga av agentens tilgjengelege formrom.}
...
\prop{6.14}{o}{Quando koplinga mellom agentar er robust, emergerer eit overordna system med større lyskjegle og meir samansette mål.}
...
\prop{6.15}{t}{Stilar og tradisjonar er deskriptive mellomkonstruksjonar utan kausal kraft. Dei skildrar dei punkta i formrommet der vandringa mellombels har stansa.}
...
\prop{7.121}{o}{At strukturen er substrat-uavhengig er eit teikn på fruktbarheit, ikkje eit prov for sanning. Agenten som skildrar formverda er sjølv ein del av ho. Det finst ingen arkimedisk punkt på utsida. Ei formlære er aldri transcendent; ho er alltid lokal og situert.}
...
# Apply terminology fixes to the new text block
new_main_text = new_main_text.replace("konfigurasjonar", "posisjonar")
new_main_text = new_main_text.replace("konfigurasjon", "posisjon")

# Re-apply the preamble and reconstruct the content range
if main_start != -1 and appendix_start != -1:
    content = content[:main_start] + new_main_text + "\n\n" + content[appendix_start:]

# Final cleanup of remaining non-nynorsk or typos
...
content = content.replace("Quando", "Når")
content = content.replace("are", "er")
content = content.replace("with éin global", "med éin global")

with open('FORMLÆRE.tex', 'w', encoding='utf-8') as f:
    f.write(content)

print("Final cleanup and space fixing complete.")
