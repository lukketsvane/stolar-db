...
# 1. REMOVE FOOTER LINE AND PREAMBLE FIXES
...
# Remove footer offset specifically if any remains
content = content.replace(r"\fancyfootoffset[L]{16mm}", "") 

# Restore \prop definition if it was missing (it should be in old_clean.tex)
# Ensure footer is 0pt everywhere
content = content.replace(r"\renewcommand{\footrulewidth}{0.4pt}", r"\renewcommand{\footrulewidth}{0pt}")

# Make footnote rule invisible
content = content.replace(r"\renewcommand{\footnoterule}{%", r"\renewcommand{\footnoterule}{")
# This is tricky because the definition is multiline. I'll use a specific substring.
old_fn_rule = r"""\renewcommand{\footnoterule}{%
  \vspace*{-3pt}%
  \noindent\llap{\rule{16mm}{0.4pt}}\rule{\linewidth}{0.4pt}%
  \vspace*{2.6pt}%
}"""
content = content.replace(old_fn_rule, r"\renewcommand{\footnoterule}{}")

# 2. CHAPTER 1 - Tractatus style with "posisjon"
# Locate the block
chap1_start_marker = r"\addcontentsline{toc}{chapter}{1 Form er ein posisjon i eit rom av moglegheiter}"
if chap1_start_marker not in content:
    chap1_start_marker = r"\addcontentsline{toc}{chapter}{1 Formverda er alt som er tilfelle}"

chap2_start_marker = r"\addcontentsline{toc}{chapter}{2 Ikkje alle posisjonar er like sannsynlege}"
...
start_idx = content.find(chap1_start_marker)
end_idx = content.find(chap2_start_marker)

if start_idx != -1 and end_idx != -1:
    # Need to keep the \clearpage if it was before the marker
...
# Locate the old 5.61 to 5.64 block
# This is a bit safer with string find
p561_m = r"\prop{5.61}{d}"
p565_m = r"\prop{5.65}{t}"
idx_p561 = content.find(p561_m)
idx_p565 = content.find(p565_m)

if idx_p561 != -1 and idx_p565 != -1:
    content = content[:idx_p561] + new_561_564 + "\n" + content[idx_p565:]
