...
Stål var i bruk i den europeiske stolklassen frå 1700-talet, men i datasettet skil ikkje H/W-fordelinga for stål-stolar pre-1925 (n = 97, μ = 1,22 ± 1,28) seg signifikant frå post-1925 (n = 86, μ = 1,32 ± 0,41) etter Welch t-test (p = 0,485). Sample-storleiken før den industrielle bruken er for liten til å gjere ein sterk falsifiseringstest av latensteorien. Det reproduserbare faktum er at den observerbare divergensen mellom stål- og tre-geometriar ikkje er ein umiddelbar konsekvens av at sveiseteknologien dukkar opp; han er, om han finst, langsam og krev fleire bekreftande analysar enn katalogdimensjonar åleine.
...
I katalogdimensjonane: djupn (CV = 0,257) mot H/D-proporsjon (CV = 1,838), ein faktor på 7×. I mesh-trekk-rommet er spreiinga endå skarpare: sphericity (CV = 0,076, sterkt kanalisert) mot råvolum (CV = 9,4, fritt), ein faktor på 123×. Gravitasjonen og kroppen kanaliserer kompaktheita. Den absolutte storleiken er fri.
...
**6. Forma ber materialet i seg, men berre svakt**  
Materialgruppe kan predikerast frå form, men berre svakt: i mesh-trekk-rommet deler ein stol berre marginalt fleire materialklassifiseringar med dei ti nærmaste naboane sine enn det den tilfeldige basisraten ville gje (excess +0,048 over basis 0,694). Signaturen er probabilistisk og delvis latent. Den sterke versjonen, om ein umiddelbar geometrisk divergens etter at sveiseteknologien dukkar opp, finn ingen statistisk støtte i dette korpuset.
...
*Data: STOLAR-databasen, n = 2 048 (1664 med full geometri, 2202 med 3D-mesh). Analyse: `analysis/formlare_analyse.py` for katalogdimensjonar, `analysis/test_hypotheses.py` og `analysis/extract_mesh_features.py` for mesh-trekk. Figurar: `analysis/figures/`.*
