...
\subsection{Ergonomisk band: kroppen sine harde grenser}

Kor mykje avgrensar kroppen? Me definerer eit \textbf{ergonomisk band}: det dimensjonsintervallet som er kompatibelt med ein sitjande vaksen. For setehogde er bandet 38--52 cm: 80\,\% av stolane ligg innanfor. For breidde (40--65 cm): 76\,\%. For djupn (38--55 cm): berre 56\,\%. Djupn har meir designfridom enn setehogde, trass i at ANOVA viste at djupn er den mest funksjonsdeterminerte dimensjonen. Paradokset lost seg slik: djupn varierer \emph{lite relativt} (laag CV), men det ergonomiske bandet er \emph{breiare}: kroppen tolererer meir variasjon i djupn enn i setehogde.
...
Databasen inneheld ogso stolar av Frank Lloyd Wright (15), Le Corbusier (6) og Jasper Morrison (6). Wright sin gjennomsnittlege stolhogde er over 100 cm (Prairie-skulen sin vertikale estetikk), medan Le Corbusier ironisk nok produserte stolar som avvik fraa sin eigen Modulor.
...
\subsection{Kobenhavn som stilmediator}

Under 434 aar med union (1380--1814) fungerte Kobenhavn som den uomstridde kulturelle hovudstaden for Noreg. Det kongelege danske kunstakademiet (grunnlagt 1754) utdanna generasjonar som forma skandinavisk design. Kaare Klint medgrunnla avdelinga for mobeldesign i 1924 med ein metode som fokuserte paa aa ``male og forenkle tidlegare mobeldesign og deretter tilpasse dei til menneskelege proporsjonar''. Klint-skulen skilde seg fraa Bauhaus: der Bauhaus feira framtida, var Klint-skulen via til aa raffinere og modernisere arketypar. I FFF-terminologi: Bauhaus dreiv \emph{utforsking} av nye materialregime (staal, royr); Klint dreiv \emph{utnytting} av eksisterande optimum (tre, tapping). Begge strategiar er gyldige navigasjonsmodusar i fitnesslandskapet.

Denne distinksjonen forklarer kvifor Empire-stilen (36 stolar, 50\,\% norske, 50\,\% danske) er den mest ``delte'' stilen i databasen: han migrerte langs unionskanalen Kobenhavn-Kristiania, formidla av danske handverkarar og moblert av karibisk mahogni.

\subsection{Substrat-avhengig trajektorie}

Ein noekkelpresisering fraa \emph{form(al) logikk} er at formposisjon i morphospace er substrat-uavhengig, men \emph{trajektorien} (vegen dit) er substrat-avhengig. Ein handverkar og ein CNC-maskin kan lage same stol, men via ulike vegar. Handverkaren justerer spontant naar han kjenner ein defekt i veden; maskinen kan det ikkje. Kvar tradisjon, kvart verktoy, kvart materiale genererer ein annan sannsynlegheitsfordeling over morphospace. Det er difor ulike nasjonar produserer ulike former sjolv under same seleksjonstrykk: dei navigerer same landskap, men med ulike substrat.
